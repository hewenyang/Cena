...
uses
  ActiveX, Math, Consts, RTLConsts, CommCtrl, FlatSB,
  WinHelpViewer{$ENDIF}, Themes, Printers, StdActns{$IFDEF MSWINDOWS};
...
//  writeln(message.msg);
                                                                              // <> 274
  if {IsAppMsg(Message) and} (Application.MainForm = Self) and (message.Msg=WM_NULL) then
  begin
    SavRet := Message.Result;
    Application.WndProc(Message);
    Message.Result := SavRet;
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            SetWindowPos(Handle, 0, 0, 0, 0, 0, SWP_HIDEWINDOW or
              SWP_NOSIZE or SWP_NOMOVE or SWP_NOZORDER or SWP_NOACTIVATE);
//lqs            SetActiveWindow(NewActiveWindow);
          end else
...
//lqs    if ActiveWindow <> 0 then SetActiveWindow(ActiveWindow);
