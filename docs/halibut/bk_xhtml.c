...
static void xhtml_donavlinks(FILE * fp, xhtmlfile * file)
{
  ...
  {
    fprintf(fp, "<a href='%s'>上一页</a> | ", xhtml_last_file->filename);
  }
  fprintf(fp, "<a href='Contents.html'>目录</a> | ");
  if (file != NULL)
  ...
  if (xhtml_next_file == NULL)
  {
    ...
    {
//      fprintf(fp, "<a href='%s'>下一页</a>", xhtml_index_filename);
      fprintf(fp, "下一页	");
    }
  } else
  {
    fprintf(fp, "<a href='%s'>下一页</a>", xhtml_next_file->filename);
  }
  fprintf(fp, "</p>\n");
}
...
static void xhtml_doheader(FILE * fp, word * title)
{
  ...
  fprintf(fp, "</title>\n");
  fprintf(fp,
          "<meta name=\"generator\" content=\"Halibut %s xhtml-backend\" charset=\"gb2312\" />\n",
          version);
  if (conf.author)
